<template>
  <div>
    ...
    >
      <div
        v-on:click="selectYourCharacter"
        ref="characterImg"
        v-html="this.svg"
        :style="{ width: this.width, height: this.height }"
        style="margin: 5px auto auto auto"
      ></div>
      <span
        v-if="isName"
        style="
...
export default {
  ...
  props: {
    ...
    defaultColorForAllAvatarElements: Object,
    group: Boolean,
  },
  methods: {
    ...
    changeShirtColor(color, proportion) {
      ...
      if (characterImgSVG) {
        this.$refs.characterImg.children[0].children[0].innerHTML += `.st0_custom_${this.id}{fill:${this.svgColor.shirt};}.st1_custom_${this.id}{fill:${this.svgColor.shirtShadow};}`;
      }
    },
    ...
    changeAccessoriesColor(color, proportion) {
      ...
      let characterImgSVG = this.$refs.characterImg.children[0];
      if (characterImgSVG) {
        this.$refs.characterImg.children[0].children[0].innerHTML += `.st8_custom_${this.id}{fill:${color};}.st9_custom_${this.id}{fill:${darkerShadeAccessories};}`;
      }
    },
    ...
    changeFaceColor(color) {
      ...
      let characterImgSVG = this.$refs.characterImg.children[0];
      if (characterImgSVG) {
        this.$refs.characterImg.children[0].children[0].innerHTML += `.st2_custom_${this.id}{fill:${this.svgColor.face};}.st3_custom_${this.id}{fill:${this.svgColor.faceShadow};}`;
      }
    },
    ...
    changeHairColor(color) {
      ...
      let characterImgSVG = this.$refs.characterImg.children[0];
      if (characterImgSVG) {
        this.$refs.characterImg.children[0].children[0].innerHTML += `.st5_custom_${this.id}{fill:${this.svgColor.hairFront};}.st4_custom_${this.id}{fill:${this.svgColor.hairBack};}`;
      }
    },
    ...
    changeBeard(pos) {
      ...
      for (
        let i = 0;
        i < this.$refs.characterImg.children[0].children.length;
        i++
      ) {
        ...
        if (current_id === "facial-hair") {
          for (
            let j = 0;
            j < this.$refs.characterImg.children[0].children[i].children.length;
            j++
          ) {
            if (j === pos) {
              this.$refs.characterImg.children[0].children[i].children[
                j
    ...
    loadSvgData() {
      if (this.svg) {
        if (!this.$refs.characterImg || !this.$refs.characterImg.children[0]) {
          setTimeout(() => {
            this.loadSvgData();
          }, 10);
          return;
    ...
    manageGlasses(position) {
      let childrenPosition =
        this.$refs.characterImg.children[0].children[position];
    ...
    manageBeards(position) {
      ...
      for (let i = 0; i < childrenPosition.children.length; i++) {
        if (childrenPosition.children[i].classList.value === "st7") {
          ...
          for (
            let j = 0;
            j < childrenPosition.children[i].children.length;
            j++
          ) {
            childrenPosition.children[i].children[j].classList.remove("st5");
            childrenPosition.children[i].children[j].classList.add(
              "st5_custom_" + this.id
            );
          }
        }
        ...
        childrenPosition.children[i].style.display = "none";
        childrenPosition.children[i].setAttribute(
          "id",
          "beards_" + i + "_" + this.id
        );
      ...
      if (characterImgSVG) {
        for (let i = 0; i < characterImgSVG.children.length; i++) {
          ...
          if (![""].includes(classValue)) {
            ...
            characterImgSVG.children[i].classList.add(
              classValue + "_custom_" + this.id
            );
          }
  ...
  mounted() {
    if (!this.isName) this.characterPositionWidth = this.width;
    this.isLoaded = true;
    this.$nextTick(() => {
      this.loadSvgData();
    });
